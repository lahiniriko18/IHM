            ...
            try:
                ...
                if typeFichier == 1:
                    ...
                    for i, horaire in enumerate(ligneWb[3]):
                        verif = (i < (nbCol // 2) and (i - 1) % nbCase == 0) or (
                            i > (nbCol // 2) and (i - 2) % nbCase == 0
                        )
                        if verif:
                            ligneDonne = [horaire]
                            for lignes in ligneWb[4:]:
                                for k in range(nbCase):
                                    ligneDonne.append(
                                        lignes[i + k] if lignes[i + k] else "vide"
                                    )
                ...
                if not all(col in df.columns for col in colonnes_requis):
                    return Response(
                        {
                            "erreur": "Format invalide. Colonne requis: 'Horaire','Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi'"
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                for i, ligne in df.iterrows():
                    ...
                    if ligne["Horaire"] != "vide":
                        ...
                        while j < len(colonnes):
                            ...
                            k = 1
                            while (
                                j + k < len(colonnes) and colonnes[j + k] not in jours
                            ):
