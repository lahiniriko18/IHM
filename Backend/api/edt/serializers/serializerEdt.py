        ...
        if self.instance:
            if salle.numSalle and salle.numSalle != self.instance.numSalle:
                if salle and not salle.statut:
                    raise serializers.ValidationError(
                        {
                            "erreur": f"Le salle {salle.nomSalle} n'est pas libre dans ce horaire !"
                        }
                    )
        else:
            if salle and not salle.statut:
...
class ClasseSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        classe = Classe.objects.filter(
            pk=data["numClasse"], niveau=data["niveau"], groupe=data.get("groupe")
        ).exists()
        if not classe:
...
class ParcoursSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        parcours = Parcours.objects.filter(
            ...
            codeParcours=data.get("codeParcours"),
            numMention=data["numMention"].numMention,
        ).first()
        if not parcours:
...
class HoraireSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        if data["heureDebut"] >= data["heureFin"]:
            raise serializers.ValidationError(
                {"erreur": "L'heure de début ne doit pas supérieur à heure de fin !"}
            )
        return data
...
class SeanceSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        enseigner = Enseigner.objects.filter(
            numProfesseur=professeur.numProfesseur, numMatiere=matiere.numMatiere
        ).exists()

        if not enseigner:
            raise serializers.ValidationError(
                {"erreur": "Ce professeur ne correspond pas par ce matière !"}
            )

        if not salle.statut:
            raise serializers.ValidationError(
                {"erreur": "Ce salle n'est pas libre dans ce horaire !"}
            )
        edt = salle.edts.filter(
            date=datetime.strptime(dateJour, "%d-%m-%Y").date(),
            heureDebut__lte=horaire["heureDebut"],
            heureFin__gte=horaire["heureFin"],
        ).exists()
        if edt:
            raise serializers.ValidationError(
                {"erreur": "Ce salle n'est pas libre dans ce date et horaire !"}
            )
    ...
    def validate(self, data):
        ...
        for jour in jours:
            ...
            if not isinstance(data.get(jour), list):
                raise serializers.ValidationError(
                    {"erreur": f"Le contenu du jour {jour} doit contenir un tableau !"}
                )
            if len(data.get(jour)) < 2:
                raise serializers.ValidationError(
                    {"erreur": f"Le jour {jour} doit contenir au moins deux séances !"}
                )
            donnees[jour] = []
            for i, seance in enumerate(data.get(jour)):
                if isinstance(seance, dict) and seance:
                    serializer = SeanceSerializer(
                        data=seance,
                        context={"date": dates.get(jour), "horaire": horaire},
                    )
                    ...
                    raise serializers.ValidationError(
                        {"erreur": "Format de données par chaque case invalide !"}
                    )
        return donnees
...
class EdtTableSerializer(serializers.Serializer):
    ...
    def validate_titre(self, value):
        ...
        if len(value) < 2:
            raise serializers.ValidationError(
                {"erreur": "Format de la titre invalide !"}
            )
        ...
        if len(dsDonnee) != 6:
            raise serializers.ValidationError(
                {"erreur": "L'emploi du temps doit contenir le date de Lundi au Samedi"}
            )
        ...
        classe = ClasseSerializer(
            Classe.objects.filter(niveau=npDonnee.niveau), many=True
        ).data
    ...
    def validate(self, data):
        ...
        serializer = ContenuSerializer(
            data=data["contenu"], many=True, context={"dates": titre[0]}
        )
