...
class MatiereView(APIView):
        ...
        if serializer.is_valid():
            ...
            if niveauParcours and len(niveauParcours) > 0:
                ...
                if serializerPossede.is_valid():
                    ...
                    return Response(
                        serializerPossede.errors, status=status.HTTP_400_BAD_REQUEST
                    )
            if professeurs and len(professeurs) > 0:
                donneeEns = []
                for professeur in professeurs:
                    donneeEns.append(
                        {"numProfesseur": professeur, "numMatiere": matiere.numMatiere}
                    )
                print(donneeEns)
            ...
            return Response(
                MatiereSerializer(matiere).data, status=status.HTTP_201_CREATED
            )
    ...
    def put(self, request, numMatiere):
        try:
            ...
            return Response(
                {"erreur": "Matière introuvable"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if serializer.is_valid():
            ...
            if isinstance(niveauParcours, list):
                ...
                for np in niveauParcours:
                    if np not in npPossede:
                        donneePossede.append(
                            {"numNiveauParcours": np, "numMatiere": matiere.numMatiere}
                        )
                ...
                if len(donneePossede) > 0:
                    serializerPossede = PossederSerializer(
                        data=donneePossede, many=True
                    )
                    if serializerPossede.is_valid():
                        serializerPossede.save()
                    else:
