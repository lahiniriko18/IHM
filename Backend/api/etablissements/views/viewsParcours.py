...
class ParcoursView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                ParcoursSerializer(parcours).data, status=status.HTTP_201_CREATED
            )
    ...
    def put(self, request, numParcours):
        try:
            ...
            return Response(
                {"erreur": "Parcours introuvable"}, status=status.HTTP_404_NOT_FOUND
            )
...
class ParcourDetailView(APIView):
    def post(self, request):
        ...
        response = serviceCrud.suppressionMutlipe(
            request.data, "numParcours", "Parcours"
        )
