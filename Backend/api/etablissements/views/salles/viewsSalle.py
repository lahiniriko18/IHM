...
class SalleView(APIView):
    ...
    def put(self, request, numSalle):
        try:
            ...
            return Response(
                {"erreur": "Salle introuvable"}, status=status.HTTP_404_NOT_FOUND
            )
...
class SalleEdtView(APIView):
    def post(self, request):
        ...
        numSalles = Edt.objects.filter(
            Q(date=dateVerif)
            & Q(
                Q(heureDebut__range=(heureDebut, heureFin))
                | Q(heureFin__range=(heureDebut, heureFin))
            )
        ).values_list("numSalle", flat=True)
