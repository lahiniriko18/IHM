...
from ..models import Edt,Professeur
...
class ServiceMailEdtProfesseur:
    def donneeProfesseurEdt(self, numEdts):
        numProfesseurs=list(Edt.objects.filter(pk__in=numEdts).values_list('numProfesseur',flat=True).distinct())
        donnees=[]
        sexes={
            "masculin":"Mr",
            "féminin":"Mme"
        ...
        if numProfesseurs:
            for numProf in numProfesseurs:
                edts=Edt.objects.filter(numEdt__in=numEdts, numProfesseur=numProf).order_by('heureDebut')
                ...
                for edt in edts:
                    ...
                    heureDebutStr=f"{int(heureDebut.hour):02d}h" if int(heureDebut.minute)==0 else f"{int(heureDebut.hour):02d}h{int(heureDebut.minute):02d}"
                    heureFin=edt.heureFin
                    heureFinStr=f"{int(heureFin.hour):02d}h" if int(heureFin.minute)==0 else f"{int(heureFin.hour):02d}h{int(heureFin.minute):02d}"
                    d={
    ...
    def distribuerMail(self, buffer, professeur, titre):
        ...
        email.attach(f'{professeur['nomCourant']}Edt.xlsx', buffer.read(), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        email.send()
