...
class EdtProfesseurSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        semaine=[datetime.strftime(lundi+timedelta(days=i), "%d-%m-%Y") for i in range(6)]
        print(semaine)
        donnees['semaine']=semaine
        
        if anneeDebut != anneeFin:
            donnees['titre2']=f"{jourDebut:02d} {moisDebut} {anneeDebut} au {jourFin:02d} {moisFin} {anneeFin}"
        elif moisDebut != moisFin:
            donnees['titre2']=f"{jourDebut:02d} {moisDebut} au {jourFin:02d} {moisFin} {anneeFin}"
        else:
