...
function Parcours() {
  ...
  const listeParcours = Array.from({ length: 16 }, (_, i) => `S${String(i + 1).padStart(3, '0')}`);
        ...
        >
          <div className="bg-white w-[90%] sm:w-[70%] md:w-[50%] lg:w-[30%] max-h-[90%] overflow-y-auto p-5 rounded-lg shadow-lg space-y-4">
            ...
            <div className="flex flex-col w-full">
              <label className="font-semibold text-sm mb-1">Identifiant de la Parcours</label>
              <input
                type="text"
                value={idParcours}
                onChange={(e) => setIdParcours(e.target.value)}
            ...
            <div className="w-full flex justify-center">
              <button
                ...
                onClick={() => {
                  console.log(isadd ? "Parcours ajoutée" : "Parcours modifiée", { idParcours, nomParcours, lieuParcours });
                  setIsclicked(false);
                }}
              >
            ...
            <tbody className="text-sm">
              {currentData.map((Parcours, index) => (
                <tr key={index} className="hover:bg-gray-100 border-b">
                  ...
                  <td className="px-4 py-2 text-center">{Parcours}</td>
