...
function Professeur() {
  ...
  const listeProfesseur = Array.from({ length: 16 }, (_, i) => `S${String(i + 1).padStart(3, '0')}`);
  const nombreElemParParge = 8;
  const [pageActuel, setPageActuel] = useState(1);

  const totalPages = Math.ceil(listeProfesseur.length / nombreElemParParge);
  const currentData = listeProfesseur.slice((pageActuel - 1) * nombreElemParParge, pageActuel * nombreElemParParge);
        ...
        >
          <div className="bg-white w-[90%] sm:w-[70%] md:w-[50%] lg:w-[30%] max-h-[90%] overflow-y-auto p-5 rounded-lg shadow-lg space-y-4">
            ...
            <div className="flex flex-col w-full">
              <label className="font-semibold text-sm mb-1">Nom de la professeur</label>
              <input
                type="text"
          ...
          <table className="table-auto w-full border-collapse">
            <thead>
              <tr className="bg-blue-500 text-white text-sm">
                ...
                <th className="px-4 py-4">Nom de la Professeur</th>
            ...
            <tbody className="text-sm">
              {currentData.map((Professeur, index) => (
                <tr key={index} className="border-b transition-all duration-300 hover:scale-105 hover:shadow-lg hover:bg-gray-100">
                  ...
                  <td className="px-4 py-2 text-center">RAKOTOVAO JEAN PAUL</td>
                  <td className="px-4 py-2 text-center">Fianarantsoa</td>
                  <td className="px-4 py-2 text-center">Docteur</td>
                  <td className="px-4 py-2 text-center">M</td>
                  <td className="px-4 py-2 text-center">0340721283</td>
