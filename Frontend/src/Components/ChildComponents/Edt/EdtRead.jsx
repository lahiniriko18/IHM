...
function EdtRead() {
  ...
  return (
    <div className={`${isReduire ? "left-20" : "left-56"} fixed right-0 top-14 p-5 h-screen overflow-auto max-w-screen-xl bg-white z-40 transition-all duration-700`}>
      <div className="flex flex-col gap-1 h-full ">
        ...
        <div className="flex justify-between items-center sticky top-0 w-full  z-30">
          <span className="text-blue-600 font-bold flex flex-row items-center z-50">Niveau :
            <Creatable
              isClearable
              isMulti
              ...
              value={optionsNiveau.filter((option) => (ObjectParametre.numNiveauParcours || []).includes(option.value))}
              className="text-sm"
            /></span>
          <div className="flex gap-2 items-center  pe-8">
            <div className='flex items-center'>
              <p className='w-40'>Date début : </p>
              <input type="date" value={ObjectParametre.dateDebut || ""} onChange={handleDateChange} name="dateDebut" id="dateDebut" className="border border-gray-300 p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-400 transition" />
            </div>
            <div className='flex items-center'>
              <p className='w-40'>Date fin : </p>
              <input
                type="date" value={ObjectParametre.dateFin || ""} readOnly onChange={() => setObjectParametre({ ...ObjectParametre, dateFin: e.target.value })} className="border border-gray-300 p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-400 transition" name='dateFin' id="dateFin"
              />
            </div>
          </div>
          <div>
        ...
        <div className="h-[73%]  w-full m-4">
          {
            isLoading ? (
              ...
              <div className="w-full h-40 flex flex-col items-center justify-center mt-[10%]">
                ...
                <p className='text-gray-400'>Veuillez sélectionner au moins un niveau</p>
              </div>
            ) :
              modele === 1 ? (
