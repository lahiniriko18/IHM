  ...
  const sendData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://127.0.0.1:8000/api/matiere/ajouter/",
        formData
      );
  ...
  const removeMatiere = async (id) => {
    try {
      const response = await axios.delete(
        `http://127.0.0.1:8000/api/matiere/supprimer/${parseInt(id)}`
      );
  ...
  const putData = async () => {
    ...
    try {
      const response = await axios.put(
        `http://127.0.0.1:8000/api/matiere/modifier/${id}`,
        formData
      );
      if (response.status !== 200) {
        throw new Error("Erreur code : " + response.status);
    ...
    <>
          ...
          <div className="bg-white w-[90%] sm:w-[70%] md:w-[50%] lg:w-[30%] max-h-[90%] overflow-y-auto p-5 rounded-lg shadow-lg space-y-4">
            ...
            <div className="flex flex-col w-full">
              ...
              <Creatable
                ...
                className="text-sm"
              />
              {error.status && error.composant === "prof" && (
                <p className="text-red-600 text-sm">{error.message}</p>
              )}
            </div>
            <div className="flex flex-col w-full">
            ...
            <div className="flex flex-col w-full">
              ...
              <input
                ...
                onChange={(e) =>
                  setDataMatiere({
                    ...dataMatiere,
                    codeMatiere: e.target.value,
                  })
                }
                className="border border-gray-300 p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
              />
              {error.status && error.composant === "codeMatiere" && (
                <p className="text-red-600 text-sm">{error.message}</p>
              )}
            </div>
                ...
                onClick={() => {
                  if (dataMatiere.nomMatiere.trim() === "") {
                    setError({
                      error,
                      status: true,
                      composant: "nomMatiere",
                      message: "Le nom du matiere ne peut pas etre vide",
                    });
                    ...
                    setError({
                      ...
                      status: true,
                      composant: "prof",
                      message: "Selectionner au moins un professeur",
                    });
                  } else {
                    if (isadd) {
        ...
        >
          <div className="bg-white w-[90%] sm:w-[70%] md:w-[50%] lg:w-[35%] max-h-[90%] overflow-y-auto p-5 rounded-lg shadow-lg space-y-4">
            ...
            <div className="flex flex-row gap-2">
              <img src="/Icons/attention.png" alt="Attention" />
              <p>
                Pour suivre la creation de matiere ,il faut au moins un
                professeur dans la section professeur
              </p>
            </div>
      ...
      >
        <div className="flex justify-between w-full">
          ...
          >
            <img src="/Icons/plus-claire.png" alt="Plus" className="w-6 h-6" />{" "}
            Nouveau
          </button>
        </div>
        {isLoading ? (
                        ...
                        <button className="p-1 rounded hover:bg-gray-200">
                          <img
                            ...
                            onClick={async () => {
                              setIsclicked(true);
                              setisadd(false);
                              await editMatiere(Matiere.numMatiere);
                            }}
                          />
                        </button>
                        <button
                          ...
                          onClick={() =>
                            confirmerSuppression(Matiere.numMatiere)
                          }
                        >
                          <img
