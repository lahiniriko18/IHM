...
function Edt() {
  ...
  const verifierEdt = async () => {
    const data = {
      dateDebut: formatDateToDDMMYYYY(dataEdt.date_debut),
      numNiveauParcours: dataEdt.niveau,
    };
    try {
  ...
  const removeEdt = async () => {
    ...
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/api/edt/supprimer/liste/`,
        formdata
      );
  ...
  const versAFfichage = (dataEdtToSend) => {
    navigate("/edt/affichage-edt", {
      state: { ObjectParam: dataEdtToSend },
    });
  };
  ...
  const currentData = listeEDT.slice(
    ...
    <>
      ...
      {isclicked ? (
        ...
        >
          <div
            className={
              " w-[90%] sm:w-[70%] md:w-[50%] lg:w-[30%] max-h-[90%]  bg-white overflow-y-auto p-5 rounded-lg shadow-lg space-y-4 "
            }
          >
              ...
              <div className={`w-full`}>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <Creatable
                    ...
                    className="text-sm"
                  />
                  {error.status && error.composant === "niveau" && (
                    <p className="text-red-600 text-sm">{error.message}</p>
                  )}
                </div>
                ...
                {error.status && error.composant === "creation" && (
                  <p className="text-red-600 text-sm">{error.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="w-full flex justify-center">
              <button
                ...
                onClick={async () => {
                  if (
                    dataEdt.date_debut.trim() == "" ||
                    dataEdt.date_fin.trim() == ""
                  ) {
                    setError({
                      status: true,
                      composant: "date_debut",
                      message: "La date ne peut pas vide",
                    });
                  } else if (!dataEdt.niveau) {
                    setError({
                      status: true,
                      composant: "niveau",
                      message: "Le niveau ne peut pas vide ",
                    });
                  } else if (!dataEdt.mode_creation) {
                    setError({
                      status: true,
                      composant: "creation",
                      message: "Il faut choisir la modele de creation",
                    });
                  } else {
      ...
      <div className="absolute top-0 left-[25%]  w-[60%]  h-14 flex justify-center items-center z-[51]">
        ...
        />
        <img
          src="/Icons/rechercher.png"
          alt="Search"
          className="w-6 absolute left-[26%]"
        />
      </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex justify-between items-center w-full">
            <h1 className="font-bold">
              Liste des emplois du temps enregistrées
            </h1>
            <button
                        ...
                        <td className="px-4 py-2 flex justify-center items-center gap-2">
                          <button className="p-1 rounded hover:bg-gray-200">
                            <img
                              ...
                              onClick={() => {
                                if (
                                  Array.isArray(EDT.numEdts) &&
                                  EDT.numEdts.length > 0
                                ) {
                                  const newNumEdtUpdate = EDT.numEdts.slice();
                                  const newDataEdt = {
                                    ...dataEdt,
                                    action: "edit",
                                    numEdtUpdate: newNumEdtUpdate,
                                  };
                                  setDataEdt(newDataEdt);
                                  navigate("/edt/edit-edt", {
                                    state: { objectStateEdt: newDataEdt },
                                  });
                                } else {
                                  console.log(
                                    "Aucun reference de l'edt trouvé ce qui  empeche la modification"
                                  );
                                }
                              }}
                            />
                            ...
                            <img
                              ...
                              onClick={() => {
                                if (
                                  ...
                                  console.log(
                                    "Aucun reference de l'edt trouvé ce qui  empeche la suppression"
                                  );
                                }
                              }}
                            />
